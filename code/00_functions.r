...
group_data <- function(data, remove_zeros = FALSE, add_constant = FALSE) {
  ...
  for (id in unique(data$studyID)) {
    ...
    if (!is.na(sdata[1,"grouping_flags"])) {
      ...
      ## account for studies which report results separately by species, multiply N by relative density values, which sum to 1.
      if(!is.na(sdata[1,"relDensityTreatment"])) {

        sdata[,"n_treatment"] <- sdata[,"n_treatment"] * sdata[,"relDensityTreatment"]
        sdata[,"n_control"] <- sdata[,"n_control"] * sdata[,"relDensityControl"]
        sdata[,"species"] <- "combined"
