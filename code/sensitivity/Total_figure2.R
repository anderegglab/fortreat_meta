...
data <- data[data$Nis2 == 0 & data$NoTrueControl == 0 & data$mortality_attribution_uncertain == 0,]
...
A <- ggplot() +
  ...
  scale_color_manual(values = c("grey", "#377eb8", "#4daf4a")) +
  scale_x_continuous(limits = c(-0.5, 5.5)) +
  ggtitle("Treatment Effects: Survivorship") +
  annotate("text", y = "fire", x = 5, label = "***", size = 15, col = "grey") +
...
B <- ggplot() +
  ...
  annotate("text", y = "thinning", x = 4.8, label = "***", size = 15, col = "grey") +
  xlab("Log Response Ratio") +
  xlim(-0.5, 5) +
  theme_bw() +
...
ggsave("figures/sensitivity/Total_figure2.png", width = 14, height = 12)
